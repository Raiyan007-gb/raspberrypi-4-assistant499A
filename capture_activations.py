...
import sys
import valib
# device_address = "98:DA:50:01:82:33"
# bluetooth = serial.Serial("/dev/rfcomm9", 9600)
# a = 1000
...
parser.add_argument(
    ...
    type=str,
    #default="/home/raiyan/Github_Repos/openWakeWord/examples/WakeWordAudio",
    default="/mnt/ramdisk/audio_play_back",
    required=False
)
parser.add_argument(
    "--threshold",
...
'''if args.model:
    ...
    for path in model_paths:
        if args.model in path:
            model_path = path'''
if args.model:
    owwModel = Model(wakeword_models=[args.model],
    ...
    while True:
        ...
        for mdl in prediction.keys():
            if prediction[mdl] >= args.threshold:
                ...
                #bluetooth.write(str(a).encode("utf-8"))
            if activation_times.get(mdl) and (time.time() - last_save) >= cooldown \
               and (time.time() - activation_times.get(mdl)[0]) >= save_delay:
                ...
                os.remove(os.path.join(os.path.abspath(args.output_dir), fname))
